...
// One-way hash for the device secret — unlike the refresh token, we never
// need the original value back, only to check "does this match what was
// stored". SHA-256 (not a slow password hash like bcrypt/scrypt) is
// appropriate here specifically because the input is already a
// high-entropy random secret generated by the device, not a low-entropy
// human-chosen password that needs deliberate slowdown against guessing.
